...
def get_pic_data(request):
    ...
    site_info = Site.objects.get(id=id)
    minute_log_list = Minute_Log.objects.all().filter(site_id=id).order_by("-time", "-access_num")
    location_all = get_all_location()
    line_data = []
    point_data = []
    for log_tmp in minute_log_list:
        line_data.append([{"name": site_info.site_name}, {"name": log_tmp.province, "value": int(log_tmp.access_num)}])
        point_data.append({"name": log_tmp.province, "value": str(log_tmp.access_num)})

    maxmin_data = [1199, 2]
    data = {"line_data": line_data, "point_data": point_data, "maxmin_data":maxmin_data, "location_all":location_all}
...
def get_site_list(request):
    ...
    proj = Site.objects.all().filter(usergroup_id=usergroup_id)
