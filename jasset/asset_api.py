...
def shell_exec(obj_list, shell):
    resource = gen_resource(obj_list)
    ansible_instance = MyRunner(resource)
    module_args = shell
    ansible_instance.run("shell", module_args, become=True)
...
def get_asset_list_by_uid(user_id):
    user_id = int(user_id)
    if user_id == 1:
        data = Asset.objects.all().filter(if_del=0)
...
def modify_hostname(plat_id, ip, hostname, host_id, key_id, key_secret):
    ...
    cmd_modify_name = '/tmp/shell/modify_hostname_new.sh %s' % hostname
    asset = Asset.objects.get(host_id=host_id)
    shell_exec([asset], cmd_modify_name)

    if plat_id == 1:
        cmd_modify_name_show = '/home/zsc/ali-ecs.py {0} {1} ModifyInstanceAttribute InstanceId={2} InstanceName={3}'.format(key_id, key_secret, host_id, hostname)
        status_show, modify_show = commands.getstatusoutput(cmd_modify_name_show)
        return 1
    elif plat_id == 2:
        cmd_modify_name_show = '/home/zsc/u-ecs.py {0} {1} ModifyInstanceAttribute UHostId={2} Name={3}'.format(key_id, key_secret, host_id, hostname)
        status_show, modify_show = commands.getstatusoutput(cmd_modify_name_show)
        return 2