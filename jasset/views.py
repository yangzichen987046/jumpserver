...
from juser.key_api import restart_host, start_host, stop_host
...
@require_role('admin')
def group_relation_edit(request):
    """
    Group edit view
    编辑资产组
    """
    header_title, path1, path2 = u'编辑主机组用户组关联', u'主机组用户组关联管理', u'编辑主机组用户组关联'
    usergroup_id = request.GET.get('id', '')
    UserGroup_name = get_object(UserGroup, id=usergroup_id)
    UGroupAGroup = Assetgroup_Usergroup.objects.filter(usergroup_id = usergroup_id).values("assetgroup_id")
    agroup = []
    for agroup_tmp in UGroupAGroup:
        agroup.append(agroup_tmp['assetgroup_id'])

    agroup_all = AssetGroup.objects.all()
    user = UserGroup.objects.all()

    if request.method == 'POST':
        assets_select = request.POST.getlist('asset', [])
        for agroup in assets_select:
            db_save_assetgroup_usergroup(agroup, usergroup_id)
        return HttpResponseRedirect(reverse('asset_group_relation'))
    # return HttpResponse(agroup)
    return my_render('jasset/group_relation_edit.html', locals(), request)


@require_role('admin')
def     group_relation_add(request):
    """
    Group edit view
    编辑资产组
    """
    header_title, path1, path2 = u'添加主机组用户组关联', u'主机组用户组关联管理', u'新增主机组用户组关联'
    ...
    if request.method == 'POST':
        ...
        assets_select = request.POST.getlist('asset', [])
        for ugroup in users_select:
            ugroup = int(ugroup)
            if ugroup > 0:
                for agroup in assets_select:
                    db_save_assetgroup_usergroup(agroup, ugroup)

    return my_render('jasset/group_relation_add.html', locals(), request)
...
def group_list(request):
    ...
    列出资产组
    """
    header_title, path1, path2 = u'查看主机组', u'主机管理', u'查看主机组'
...
@require_role('admin')
def group_relation(request):
    """
    list asset group
    列出资产组
    """
    header_title, path1, path2 = u'查看用户主机组关联', u'主机管理', u'用户主机组关联'
    keyword = request.GET.get('keyword', '')

    asset_group_relation = Assetgroup_Usergroup.objects.all()
    asset_group_relation = Assetgroup_Usergroup.objects.order_by('usergroup_id')

    group_id = request.GET.get('id')

    return my_render('jasset/group_relation.html', locals(), request)
...
def asset_edit(request):
    ...
    if request.method == 'POST':
        ...
        is_active = True if request.POST.get('is_active') == '1' else False
        use_default_auth = request.POST.get('use_default_auth', '')
        host_name_show_new = request.POST.get('host_name_show', '')
        if_host_name_change = False
        if asset.host_name_show != host_name_show_new:
            if_host_name_change = True
            key_info = AccessKey.objects.get(key_id=asset.accesskey_id)
            modify_hostname(asset.plat_id, asset.ip, host_name_show_new, asset.host_id, key_info.key_id, key_info.key_secret)
...
def list_update(request):
    ...
    if user_id == 1:
        data_asset = Asset.objects.all().filter(if_del=0)
        data_key = AccessKey.objects.all().filter(if_del=0)
    else:
        ...
        data_asset = Asset.objects.all().filter(id__in=a_group).filter(if_del=0)
        data_key = AccessKey.objects.all().filter(group_id=user_group.usergroup_id).filter(if_del=0)
...
def get_asset_list(request):
    ...
    data = get_asset_list_by_uid(user_id)
    data = serializers.serialize('json',data)
    return HttpResponse(data)


@require_role('admin')
def restart_asset(request):
    num_id = request.GET.get('id', '')
    host_info = Asset.objects.get(id=num_id)
    if host_info:
        host_id = host_info.host_id
        key_info = AccessKey.objects.get(key_id=host_info.accesskey_id)
        key_secret = key_info.key_secret
        res = restart_host(host_info.accesskey_id, key_secret, host_id, host_info.plat_id, host_info.region_id)
        return HttpResponse(res)


@require_role('admin')
def start_asset(request):
    '''
    先做判断，用户是否有权限操作本主机
    '''
    user_id = request.session['_auth_user_id']
    asset_list_tmp = get_asset_list_by_uid(user_id)
    asset_list = []
    for asset in asset_list_tmp:
        asset_list.append(asset.id)

    num_id = request.GET.get('id', '')
    num_id = int(num_id)
    if num_id in asset_list:
        host_info = Asset.objects.get(id=num_id)
        if host_info:
            host_id = host_info.host_id
            key_info = AccessKey.objects.get(key_id=host_info.accesskey_id)
            key_secret = key_info.key_secret
            res_tmp = start_host(host_info.accesskey_id, key_secret, host_id, host_info.plat_id, host_info.region_id)
            res = {}
            if res_tmp:
                res['status'] = True
            else:
...
def stop_asset(request):
    ...
    if num_id in asset_list:
        ...
        if host_info:
            ...
            key_info = AccessKey.objects.get(key_id=host_info.accesskey_id)
            key_secret = key_info.key_secret
            res_tmp = stop_host(host_info.accesskey_id, key_secret, host_id, host_info.plat_id, host_info.region_id)
            res = {}
            if res_tmp:
                res['status'] = True
            else:
    ...
    if request.method == 'POST':
        ...
        if arg == 'all':
            ...
            for asset_id in asset_id_all:
                ...
                if asset:
                    asset_list.append(asset)
        shell_wget = 'wget -P /tmp --http-user=live400 --http-passwd=Live400Com  http://d.live400.com/shell_jumpserver.tar.gz'
        shell_exec(asset_list, shell_wget)

        shell_tar = 'tar zxvf /tmp/shell_jumpserver.tar.gz -C /tmp'
        shell_exec(asset_list, shell_tar)

        shell_mv = 'mv /tmp/shell_jumpserver /tmp/shell'
        shell_exec(asset_list, shell_mv)

        shell_init = '/tmp/shell/sys_init.sh'
        shell_exec(asset_list, shell_init)

        return HttpResponse(u'初始化成功!')
    return HttpResponse(u'初始化成功!')