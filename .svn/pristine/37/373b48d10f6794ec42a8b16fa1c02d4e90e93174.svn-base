...
def project_edit(request):
    ...
    if request.method == 'POST':
        ...
        proj_edit = Project.objects.filter(id=project_id)
        proj_edit.update(
            release_host_name=host_info.host_name_show,
            release_host_ip=host_info.ip,
            release_host_id=host_info.host_id,
...
def code_release(request):
    ...
    if request.method == 'POST':
        ...
        strtime = time.time()
        db_add_release(asset, proj_id, version, strtime)
        code_info = Upload_Code.objects.all().get(id=version)
        proj_info = Project.objects.all().get(id=proj_id)
        if code_info.release_status == 0:
            if len(asset) == 1:
                first_release(asset[0], version, code_info.proj_name, code_info.code_name, proj_info.release_host_ip, proj_info.check_url, proj_info.check_port, strtime)
            else:
                first_res = first_release(asset[0], version, code_info.proj_name, code_info.code_name, proj_info.release_host_ip, proj_info.check_url, proj_info.check_port, strtime)
                first_res = int(first_res)
                if first_res == 1:
                    common_release(list(set(asset) ^ set([asset[0]])), version, code_info.proj_name, code_info.code_name, proj_info.release_host_ip, proj_info.check_url, proj_info.check_port, strtime)
                else:
                    Detail.objects.filter(release_time=strtime).update(release_status=2)
                    return HttpResponse('首台发布失败，发布中断')
        elif code_info.release_status == 1:
            common_release(asset, version, code_info.proj_name, code_info.code_name, proj_info.release_host_ip, proj_info.check_url, proj_info.check_port, strtime)
        elif code_info.release_status == 2:
            Detail.objects.filter(release_time=strtime).update(release_status=2)
            return HttpResponse('该版本发布失败，请选择其他版本')

        return HttpResponseRedirect(reverse('release_release_detail'))
...
@require_role('admin')
def get_version_asset_list_by_proj(request):
    """
    ajax根据项目ID获取版本和主机列表
    """
    data = {}
    ...
    proj_info = Project.objects.all().get(id=proj_id)
    cluster_info = Cluster.objects.all().filter(id=proj_info.cluster_id)
    if cluster_info:
        asset_id = cluster_info[0].hosts.split('|')

        version = Upload_Code.objects.all().filter(proj_id=proj_id)
        host = Asset.objects.all().filter(id__in=asset_id).filter(if_del=0)

        host = serializers.serialize('json', host)
        version = serializers.serialize('json', version)
        data = {"host": host, "version": version}
        data = json.dumps(data)
    else:
        data = {"host": [], "version": []}
        data = json.dumps(data)
...
def release_detail(request):
    ...
    """
    user_role = {'SU': u'超级管理员', 'GA': u'组管理员', 'CU': u'普通用户'}
    header_title, path1, path2 = '发布详情', '自动化发布', '发布详情列表'
    detail_all=Detail.objects.all()
    return my_render('jrelease/release_detail.html', locals(), request)
