...
def get_project_name(code_version_id):

    upload = get_object(Upload_Code, id=code_version_id)
    if upload:
        return upload.proj_name
    else:
        return code_version_id


@register.filter(name='get_version_name')
def get_version_name(code_version_id):
...
@register.filter(name='get_release_status')
def get_release_status(release_status):
    """
    根据运行状态返回中文
    """
    if release_status == 0:
        return u"发布中"
    elif release_status == 1:
        return u"发布成功"
    elif release_status == 2:
        return u"发布失败"
