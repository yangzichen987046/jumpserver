...
def get_usergroup_by_asset(host_id):
    """
    根据主机host_id查询所属用户组
    """
    asset_detail = Asset.objects.get(host_id=host_id)
    asset_group = Asset_Group.objects.filter(asset_id=asset_detail.id)
    group = Assetgroup_Usergroup.objects.all().filter(assetgroup_id=asset_group[0].assetgroup_id)
    # return asset_group.id
    return group[0].usergroup_id
